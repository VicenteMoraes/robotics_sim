...
def test_nav2():
    ...
    sim._add(DockerLogger(target='', write_to_file=True, filename='sim.log'))
    robot = Turtlebot3(client, robot_name="turtlebot", robot_namespace="turtlebot", auto_remove=True, network=network)
    robot._add(DockerLogger(target='', write_to_file=True, filename='robot.log'))
    rviz = RVIZ(client, auto_remove=True, network=network)
    rviz._add(DockerLogger(target='', write_to_file=True, filename='rviz.log'))
