...
class Turtlebot3(Robot):
    def __init__(self, path: str = DEFAULT_PATH, command=None, tag: str = "turtlebot3", robot_name: str = "turtlebot",
                 robot_namespace: str = "turtlebot", container_name: str = "turtlebot3",
                 *args, **kwargs):
        ...
        if command is None:
            self.command = f"python3 /workdir/launch/robot_bringup.py {robot_name} {robot_namespace} {' '.join([str(x) for x in self.initial_pose])}"
